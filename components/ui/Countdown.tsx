...
/**
 * Countdown.tsx · Configurable 3–2–1 countdown with optional female voice audio
 */

import { useEffect, useRef } from "react";
import { gsap } from "gsap";
import { speakCountdownNumber } from "@/lib/soundEffects";

interface CountdownProps {
  /** Duration per digit in seconds (default 1 s) */
  duration?: number;
  /** Starting number for the countdown (default 3) */
  from?: number;
  /** Whether audio voice countdown is enabled */
  ...
  useEffect(() => {
    ...
    digits.forEach((n) => {
      tl.call(() => {
        ...
        speakCountdownNumber(n, soundEnabled);
      })
        .fromTo(
          el,
          { scale: 1.4, opacity: 0 },
  ...
  return (
    <div className="absolute inset-0 z-30 flex items-center justify-center">
      ...
      <div className="relative flex h-36 w-36 items-center justify-center">
        ...
        <div
          ref={progressRef}
          className="absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-isy-green-deep/80 px-3 py-0.5 text-[11px] font-bold text-white backdrop-blur-sm"
        >
          Bersiap…
        </div>
      </div>
